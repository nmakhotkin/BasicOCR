...
def tf_input_fn(params, is_training):
    ...
    def _input_fn():
        ...
        def _parser(example):
            ...
            w = tf.maximum(tf.cast(original_w, tf.float32),1)
            h = tf.maximum(tf.cast(original_h, tf.float32),1)
            ratio_w = tf.maximum(w / max_width, 1.0)
            ratio_h = tf.maximum(h / 32.0, 1.0)
            ratio = tf.maximum(ratio_w, ratio_h)
            nw = tf.cast(tf.math.floor(w / ratio), tf.int32)
            nh = tf.cast(tf.math.floor(h / ratio), tf.int32)
